...
#define cerr ss
    ...
    desc.add_options()
        ...
        ("duration", po::value<double>(&response_duration)->default_value(0.001), "duration of response (seconds)")
    ...
    ///////////////////////////////////
//string _dummy;
//getline(cin, _dummy); // Wait for ENTER
    window = initscr();
    ...
    if (test_iterations > 0)    // Force certain settings during test mode
    {
        no_delay = false;
    ...
    //const int flush_count = 16; // Flush the half-band filters on the hardware

//    std::vector<std::complex<float> > response_buff(/*tx_stream->get_max_num_samps()*/response_length, std::complex<float>(output_scale, output_scale));
//    for (int i = 0; i < flush_count; ++i)
//        response_buff.push_back(std::complex<float>(0.0f, 0.0f));
    ...
    //std::ofstream outfile("input.cfile", std::ofstream::binary);
    //std::ofstream threshold_file("threshold.cfile", std::ofstream::binary);
    ...
    while (not stop_signal_called/* and (num_requested_samples != num_total_samps or num_requested_samples == 0)*/) {
        ...
        if (level_calibration_countdown > 0)
        {
            ...
            if (level_calibration_countdown <= 0)
            {
                if (level_calibration_stage_2 == false)
                {
                    ...
                    cout << boost::format("Phase #1: Ave low: %.3f (#%d), ave high: %.3f (#%d), threshold: %.3f") % ave_low % ave_low_count % ave_high % ave_high_count % threshold << endl;
                    FLUSH_SCREEN();
                ...
                else
                {
                    ...
                    threshold = ave_low + ((ave_high - ave_low) * trigger_level);
                    cout << boost::format("Phase #2: Ave low: %.3f (#%d), ave high: %.3f (#%d), threshold: %.3f") % ave_low % ave_low_count % ave_high % ave_high_count % threshold << endl;
                    cout << endl;
                    FLUSH_SCREEN();
        ...
        if (num_total_samps == 0)
        {
            if (test_iterations > 0)
            {
                ...
                {
                    assert(optimize_simulation_rate || adjust_simulation_rate);
        ...
        for (size_t n = 0; n < num_rx_samps; n++)
        {
            ...
            if (simulate_duration > 0)
                new_state = (((num_total_samps + n) % simulate_duration) == 0);
            else
                new_state = (f >= threshold);    // TODO: Just measure difference in fall

            //threshold_file.write((const char*)&new_state, 1);   // FIXME
            ...
            if ((new_state == false) && (input_state))
            {
                ...
                if ((test_iterations > 0) && (skip_iterations > 0) && (statsCurrent.skipped == 0) && (skip_iterations == statsCurrent.detected))
                {
                    memset(&statsCurrent, 0x00, sizeof(STATS));
                ...
                const size_t length_to_send = /*response_buff.size()*/response_length + (flush_count - 1);
                size_t num_tx_samps = tx_stream->send(/*&response_buff.front()*/pResponse, length_to_send, tx_md, timeout);
                if (num_tx_samps < length_to_send)
                {
                    cerr << "Send timeout" << std::endl;
                    FLUSH_SCREEN_NL();
                }
                ...
                if (skip_eob == false)
                {
                    ...
                    tx_md.has_time_spec = false;
                    //tx_md.has_time_spec = true;
                    //tx_md.time_spec = rx_md.time_spec + uhd::time_spec_t(0, n+response_buff.size(), sample_rate) + uhd::time_spec_t(delay);
                    //tx_stream->send("", 0, tx_md);
                    //static std::complex<float> last_sample(0.0f, 0.0f);
                    //tx_stream->send(&last_sample, 1, tx_md);
                    const size_t eob_length_to_send = 1;
                    size_t eob_num_tx_samps = tx_stream->send(/*&response_buff[response_buff.size() - 1]*/&pResponse[length_to_send], eob_length_to_send, tx_md);
...
/*
                ...
                while (not got_async_burst_ack and usrp->get_device()->recv_async_msg(async_md, 0)) {   // timeout
                    got_async_burst_ack = (async_md.event_code == uhd::async_metadata_t::EVENT_CODE_BURST_ACK);
                }
                //cout << (got_async_burst_ack? "success" : "fail") << std::endl;
                if (got_async_burst_ack == false) {
                    cout << "Missing async burst ACK" << std::endl;
                    FLUSH_SCREEN();
                }
*/
        ...
        if (test_iterations > 0)
        {
                ...
                ((test_iterations_is_sample_count) && (num_total_samps_test > test_iterations)))
            {
                ...
                cout << boost::format("Delay now: %.6f (previous delay %.6f scored %f%% [%ld / %ld])") % delay % statsPrev.delay % score % (statsPrev.detected - statsPrev.missed) % statsPrev.detected;
                if (old_simulate_duration != simulate_duration)
                    cout << boost::format(" [Simulation rate now: %f Hz (%ld samples)]") % simulate_frequency % simulate_duration;
                FLUSH_SCREEN();
